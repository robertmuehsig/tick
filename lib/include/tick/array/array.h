...
class Array : public BaseArray<T> {
 ...
 public:
   ...
   */
  template <typename K = T, typename F>
  typename std::enable_if<std::is_same<T, std::atomic<K>>::value>::type
  ...
  template <class K>
  void
  mult_incr(const BaseArray<T> &x, const K a);
...
void Array<T>::sort(Array<ulong> &index, bool increasing) {
  if (increasing)
    sort_function<K>(index, less_comparator<T>);
  else
    sort_function<K>(index, greater_comparator<T>);
}
...
void Array<T>::sort_abs(Array<ulong> &index, bool increasing) {
  if (increasing)
    sort_function<K>(index, less_comparator_abs<T>);
  else
    sort_function<K>(index, greater_comparator_abs<T>);
}
...
template <class K>
void
Array<T>::mult_incr(const BaseArray<T> &x, const K a) {
  if (this->size() != x.size()) {
    TICK_ERROR("Vectors don't have the same size.");
  } else {
    if (x.is_sparse()) {
      for (uint64_t j = 0; j < x.size_sparse(); j++) {
        ...
        set_data_index(x_index_j,
          this->template get_data_index<K>(x_index_j)
            + (x.template get_data_index<K>(j) * a));
      }
    } else {
